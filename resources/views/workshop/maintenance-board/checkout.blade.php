...
@section('content')
    <div x-data="{
            ...
            pendingLinesTotal() {
                @if($isAnticipo ?? false)
                    const anticipoLine = this.pendingLines.find(l => l.detail_id === 'anticipo');
                    return anticipoLine ? this.lineSubtotal(anticipoLine) : 0;
                @else
        ...
        @endphp
        <x-common.page-breadcrumb :pageTitle="'Venta y cobro · ' . $checkoutOsLabel" :crumbs="[
            ['label' => 'Tablero de Mantenimiento', 'url' => route('workshop.maintenance-board.index')],
            ['label' => $checkoutOsLabel . ' | Venta y cobro'],
        ]" />

        <x-common.component-card :title="($deliversOnConfirm ?? false) ? 'OS Finalizada - Venta y cobro' : ($isAnticipo ? 'Pago Anticipado de OS' : 'Venta y cobro de OS')" :desc="($deliversOnConfirm ?? false) ? 'Factura servicios pendientes, agrega productos y registra el pago para entregar la unidad.' : ($isAnticipo ? 'Registra un adelanto de pago para la orden de servicio. Se generará un comprobante por el monto que ingreses.' : 'Genera la venta, emite el comprobante y registra cobro o crédito sin finalizar el servicio.')">
            @if (session('status'))
            ...
            <form method="POST"
                ...
                <div class="overflow-hidden rounded-xl border border-slate-200">
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide">Descripcion
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <template x-if="pendingLines.length === 0">
                                <tr>
                                    <td colspan="4" class="px-3 py-5 text-center text-sm text-slate-500">No hay líneas
                                        pendientes.</td>
                                </tr>
                            </template>
                            <template x-for="line in pendingLines" :key="`pending-${line.detail_id}`">
                                <tr>
                                    <td class="px-3 py-2 text-sm text-slate-700">
                                        ...
                                        <span
                                            class="ml-2 rounded bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600"
                                            x-text="line.line_type"></span>
                                    </td>
                                    <td class="px-3 py-2 text-center text-sm text-slate-700"
                                        x-text="Number(line.qty || 0).toFixed(2)"></td>
                                    <td class="px-3 py-2 text-center text-sm text-slate-700">
                                        <template x-if="'{{ $isAnticipo ?? false ? 1 : 0 }}' == '1' && String(line.detail_id) === 'anticipo'">
                                            <div class="flex items-center justify-center gap-1">
                                                <span class="text-slate-500 font-semibold">S/</span>
                                                <input type="number" step="0.01" min="0.01" x-model="line.unit_price" @input="syncAmount()" class="h-9 w-24 rounded-lg border border-indigo-300 bg-indigo-50 px-2 text-center text-sm font-bold text-indigo-700 outline-none transition focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                                            </div>
                                        </template>
                                        <template x-if="!('{{ $isAnticipo ?? false ? 1 : 0 }}' == '1' && String(line.detail_id) === 'anticipo')">
                                            <span x-text="`S/ ${Number(line.unit_price || 0).toFixed(2)}`" :class="line.unit_price < 0 ? 'text-red-600' : ''"></span>
                                        </template>
                                    </td>
                                    <td class="px-3 py-2 text-right text-sm font-semibold text-slate-800"
                                        x-text="`S/ ${lineSubtotal(line).toFixed(2)}`"></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                        ...
                        <template x-for="(line, index) in productLines" :key="`product-line-${index}`">
                            <div class="grid grid-cols-12 gap-2 rounded-lg border border-indigo-100 bg-white p-2">
                                <div class="col-span-12 md:col-span-6">
                                    <input type="hidden" :name="`product_lines[${index}][product_id]`"
                                        :value="line.product_id">
                                    <select x-model="line.product_id" @change="onProductChange(index)"
                                        class="h-9 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none transition focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                                        <option value="">Selecciona un producto...</option>
                                        <template x-for="p in productsCatalog" :key="p.id">
                                            <option :value="String(p.id)" x-text="p.description"></option>
                                        </template>
                                    </select>
                                </div>
                                <div class="col-span-12 flex gap-2 md:col-span-6">
                                    <input type="number" step="0.01" min="0.01" :name="`product_lines[${index}][qty]`"
                                        x-model="line.qty" @input="syncAmount()"
                                        class="h-9 w-20 rounded-lg border border-slate-200 px-2 text-center text-sm outline-none transition focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                                        placeholder="Cant.">
                                    <input type="number" step="0.01" min="0" :name="`product_lines[${index}][unit_price]`"
                                        x-model="line.unit_price" @input="syncAmount()"
                                        class="h-9 w-24 flex-1 rounded-lg border border-slate-200 px-2 text-right text-sm outline-none transition focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                                        placeholder="P. Unit">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-5">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="saleHeaderSeries" readonly tabindex="-1"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-slate-50 px-3 text-sm font-semibold text-slate-700">
                    </div>
                    <div>
                        <label
                            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Correlativo</label>
                        <input type="text" x-model="saleHeaderNumber" readonly tabindex="-1"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-slate-50 px-3 text-sm font-semibold text-slate-700">
                    </div>
                    <div>
                        <label
                            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Condición</label>
                        <select x-model="paymentType" @change="onPaymentTypeChange()" name="payment_type"
                            class="h-11 w-full rounded-lg border border-gray-300 px-3 text-sm">
                    ...
                    <div>
                        ...
                        <select x-ref="cashRegisterSelect" x-model="selectedCashRegisterId"
                            @change="refreshSaleHeaderPreview()" name="cash_register_id" required
                            class="h-11 w-full rounded-lg border border-gray-300 px-3 text-sm">
                ...
                <div x-show="isInvoiceDocumentSelected()" x-cloak
                    class="grid grid-cols-1 gap-3 rounded-xl border border-amber-200 bg-amber-50/60 p-4 md:grid-cols-3">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Estado de
                            factura</label>
                        <select x-model="billingStatus" @change="syncInvoiceBillingFields(); refreshSaleHeaderPreview()"
                            name="billing_status"
                    ...
                    <div x-show="billingStatus === 'INVOICED'" x-cloak>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Serie</label>
                        <input type="text" name="invoice_series" x-model="invoiceSeries"
                            ...
                            class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm" placeholder="001">
                    </div>
                    <div x-show="billingStatus === 'INVOICED'" x-cloak>
                        <label
                            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Correlativo</label>
                ...
                <div x-show="!isDebtPaymentSelected() && nothingToCollect()" x-cloak
        ...
        : 'No se registran medios de pago. Elige el documento de venta y confirma para facturar (si aplica); la OS seguirá en su estado actual.' }}
                ...
                <div x-show="!isDebtPaymentSelected() && !nothingToCollect()" x-cloak data-gsa-skip="true"
                    class="overflow-visible rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div
                        class="flex flex-col gap-2 border-b border-slate-200 bg-slate-50 px-4 py-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            <p class="text-sm font-semibold text-slate-800">Desglose de cobro</p>
                            <p class="text-xs text-slate-500">Puedes combinar uno o varios métodos de pago en la misma
                                operación.</p>
                        </div>
                        <button type="button" @click="addPaymentRow(true)"
                        ...
                        <template x-for="(row, index) in paymentRows" :key="`payment-row-${index}`">
                            <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-3">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500"
                                                x-text="row.kind === 'card' ? 'Requiere tarjeta y puede usar pasarela.' : (row.kind === 'wallet' ? 'Requiere seleccionar la billetera digital.' : 'No requiere detalles adicionales.')">
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div class="grid items-end gap-3"
                                    ...
                                    <div class="min-w-0">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Método</label>
                                        <select x-model="row.payment_method_id" @change="onPaymentMethodChange(index)"
                                            :name="`payment_methods[${index}][payment_method_id]`" required
                                    ...
                                    <div class="min-w-0">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Monto</label>
                                        <input type="number" step="0.01" min="0.01" x-model="row.amount"
                                            :name="`payment_methods[${index}][amount]`" required
                                    ...
                                    <div class="min-w-0" x-show="row.kind === 'card'">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Tarjeta</label>
                                        <select x-model="row.card_id" :name="`payment_methods[${index}][card_id]`"
                                            x-bind:required="row.kind === 'card'" x-bind:disabled="row.kind !== 'card'"
                                            ...
                                            <template x-for="card in cardOptions" :key="`card-${card.id}`">
                                                <option :value="String(card.id)"
                                                    x-text="card.type ? `${card.description} (${card.type})` : card.description">
                                                </option>
                                            </template>
                                        </select>
                                    </div>
                                    ...
                                    <div class="min-w-0" x-show="row.kind === 'wallet'">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Billetera
                                            digital</label>
                                        <select x-model="row.digital_wallet_id"
                                            :name="`payment_methods[${index}][digital_wallet_id]`"
                                            x-bind:required="row.kind === 'wallet'" x-bind:disabled="row.kind !== 'wallet'"
                                            class="h-10 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-slate-500">
                                    ...
                                    <div class="min-w-0" x-show="row.kind !== 'card' && row.kind !== 'wallet'">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Detalle</label>
                                        <div
                                            class="flex h-10 items-center rounded-xl border border-dashed border-slate-300 bg-white px-3 text-sm text-slate-400">
                                    ...
                                    <div class="min-w-0" x-show="row.kind === 'card'">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Pasarela</label>
                                        <select x-model="row.payment_gateway_id"
                                            ...
                                            <template x-for="gateway in availableGateways(row.payment_method_id)"
                                                :key="`gateway-${row.payment_method_id}-${gateway.id}`">
                                    ...
                                    <div class="min-w-0">
                                        <label
                                            class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">Referencia</label>
                                        <input type="text" x-model="row.reference"
                    ...
                    <div class="grid gap-3 border-t border-slate-200 bg-slate-50 px-3 py-3 md:grid-cols-3">
                        ...
                        <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5">
                            ...
                            <p class="mt-1 text-lg font-bold text-emerald-700" x-text="`S/ ${chargeTotal().toFixed(2)}`">
                            </p>
                        </div>
                        <div class="rounded-xl border px-4 py-2.5"
                            ...
                            <p class="text-[11px] font-semibold uppercase tracking-[0.12em]"
                                :class="Math.abs(remainingAmount()) < 0.009 ? 'text-sky-700' : (remainingAmount() > 0 ? 'text-amber-700' : 'text-rose-700')">
                                Diferencia</p>
                            <p class="mt-1 text-lg font-bold"
                                :class="Math.abs(remainingAmount()) < 0.009 ? 'text-sky-700' : (remainingAmount() > 0 ? 'text-amber-700' : 'text-rose-700')"
                                x-text="`S/ ${remainingAmount().toFixed(2)}`"></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600">Comentario
                            cobro (opcional)</label>
                        <input type="text" name="payment_comment"
                            value="{{ old('payment_comment', 'Cobro registrado desde tablero') }}"
                            class="h-11 w-full rounded-lg border border-gray-300 px-3 text-sm">
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-right">
                    <p class="text-[11px] font-semibold uppercase tracking-wide text-emerald-700"
                        x-text="isDebtPaymentSelected() ? 'Total a registrar como deuda' : 'Total a cobrar ahora'"></p>
