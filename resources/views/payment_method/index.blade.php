...
<x-common.component-card title="Listado de métodos de pago" desc="Gestiona los métodos de pago registrados en el sistema.">
        ...
        <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex-none">
                <x-form.select-autocomplete
                    name="per_page"
                    :value="$perPage ?? 10"
                    :options="collect($allowedPerPage ?? [10, 20, 50, 100])->map(fn($n) => ['value' => $n, 'label' => $n . ' / página'])->values()->all()"
                    placeholder="Por página"
                    :submit-on-change="true"
                    inputClass="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                />
            </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-ui.link-button size="md" variant="outline" href="{{ $viewId ? route('admin.payment_methods.index', ['view_id' => $viewId]) : route('admin.payment_methods.index') }}" class="flex-1 sm:flex-none h-11 px-4 border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
    ...
    <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex-none pagination-simple">
            {{ $paymentMethods->links('vendor.pagination.forced') }}
        </div>
    </div>
