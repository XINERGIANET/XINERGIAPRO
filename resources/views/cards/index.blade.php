    ...
    <x-common.component-card title="Listado de tarjetas" desc="Gestiona las tarjetas registradas en el sistema.">
                ...
                <div class="flex-none">
                    <select name="per_page"
                        ...
                        @foreach ($allowedPerPage ?? [10, 20, 50, 100] as $size)
                            <option value="{{ $size }}" {{ ($perPage ?? 10) == $size ? 'selected' : '' }}>{{ $size }} / página</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="mt-6 flex flex-col items-center justify-center gap-4 sm:flex-row sm:justify-between border-t border-gray-100 pt-6 dark:border-gray-800">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Mostrando <span class="font-medium text-gray-700 dark:text-gray-200">{{ $cards->firstItem() ?? 0 }}</span> a <span class="font-medium text-gray-700 dark:text-gray-200">{{ $cards->lastItem() ?? 0 }}</span> de <span class="font-medium text-gray-700 dark:text-gray-200">{{ $cards->total() }}</span> registros
            </p>
