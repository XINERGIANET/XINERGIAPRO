...
@section('content')
    ...
    <div class="dashboard-toolbar flex items-center justify-between mb-6 print:hidden">
        ...
        <form method="GET" action="{{ route('dashboard') }}"
            class="dashboard-filter-form flex flex-wrap items-center gap-3 print:hidden">
            @if (request('view_id'))
                <input type="hidden" name="view_id" value="{{ request('view_id') }}">
            @endif
            <input type="date" name="date_from" value="{{ $d['dateFrom'] ?? now()->toDateString() }}"
                class="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm">
            <input type="date" name="date_to" value="{{ $d['dateTo'] ?? now()->toDateString() }}"
                class="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm">
            <button type="submit"
                class="bg-slate-800 text-white px-4 py-2 rounded-xl text-xs font-black shadow-sm flex items-center gap-2 hover:bg-slate-900 transition-all h-11">
                <i class="ri-search-line"></i> Filtrar
            </button>
            <a href="{{ route('dashboard', request('view_id') ? ['view_id' => request('view_id')] : []) }}"
                class="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-xs font-black shadow-sm flex items-center gap-2 hover:bg-slate-50 transition-all h-11">
                <i class="ri-refresh-line"></i> Limpiar
            </a>
            <a href="{{ route('dashboard', array_filter(['date_from' => now()->toDateString(), 'date_to' => now()->toDateString(), 'view_id' => request('view_id')])) }}"
                class="bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-black shadow-sm flex items-center gap-2 hover:bg-blue-700 transition-all h-11">
    ...
    <div class="space-y-6 pb-10" id="workshop-dashboard">
        ...
        <header class="hidden print:flex flex-col gap-3 mb-6 pb-4 border-b-2 border-slate-900 w-full">
            <div class="flex flex-row justify-between items-start w-full">
                <div class="flex flex-col gap-2">
                    ...
                    <h1 class="text-4xl font-black text-slate-900 tracking-tight leading-none uppercase">Reporte Operativo
                        de Taller</h1>
                    <div class="flex items-center gap-4 mt-1">
                        <p
                            class="text-sm font-bold text-slate-500 uppercase tracking-widest border-r border-slate-300 pr-4">
                            Sede: <span class="text-slate-900">{{ $dashboardData['branchName'] ?? 'Principal' }}</span>
                        </p>
                        <p
                            class="text-[10px] font-black text-white bg-slate-900 px-2 py-0.5 rounded uppercase tracking-tighter">
                ...
                <div class="text-right flex flex-col items-end gap-1">
                    <div
                        class="px-4 py-1.5 bg-slate-100 border border-slate-200 text-slate-800 text-[10px] font-black rounded-lg uppercase tracking-widest mb-2 shadow-sm">
                        Resumen Diario
                    </div>
                    <p class="text-xs font-bold text-slate-600">Fecha: <span
                            class="text-slate-900 font-black">{{ now()->format('d/m/Y') }}</span></p>
                    <p class="text-xs font-bold text-slate-600">Hora: <span
                            class="text-slate-900 font-black">{{ now()->format('H:i') }}</span></p>
                    <p class="text-[10px] text-slate-400 mt-3 italic font-medium">Generado por:
            ...
            <div class="grid grid-cols-5 gap-6 mt-4 w-full">
                <div class="p-5 bg-white border-2 border-emerald-600 rounded-2xl shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-900 mt-1">S/
                        {{ number_format($dashboardData['todayInvoiced'] ?? 0, 2) }}</p>
                </div>
                <div class="p-5 bg-white border-2 border-red-600 rounded-2xl shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-900 mt-1">S/
                        {{ number_format($dashboardData['expensesToday'] ?? 0, 2) }}</p>
                </div>
                <div class="p-5 bg-white border-2 border-blue-600 rounded-2xl shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-900 mt-1">
                        {{ number_format($dashboardData['ordersActive'] ?? 0) }}</p>
                </div>
                <div class="p-5 bg-white border-2 border-purple-600 rounded-2xl shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-900 mt-1">
                        {{ number_format($dashboardData['maintenancesWeek'] ?? 0) }}</p>
                </div>
                <div class="p-5 bg-white border-2 border-amber-500 rounded-2xl shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-900 mt-1">S/
                        {{ number_format($dashboardData['productionAmount'] ?? 0, 2) }}</p>
                </div>
            </div>
        </header>
        ...
        <section
            class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 mb-8 hero-kpi-grid print:grid-cols-5 print:gap-3 print:mb-4">
            <!-- Ingreso Hoy -->
            <article class="bg-white p-6 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-50/50">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="w-10 h-10 rounded-xl text-emerald-500 flex items-center justify-center bg-[#F4F6FA] border border-slate-100">
                        <i class="ri-line-chart-line text-xl"></i>
                    </div>
                    <div
                        class="flex items-center gap-1 text-[10px] font-black {{ $incomeGrowth >= 0 ? 'text-emerald-500' : 'text-red-500' }}">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 leading-none mb-1">S/
                        {{ number_format((float) ($d['todayInvoiced'] ?? 0), 2) }}</p>
            ...
            <article class="bg-white p-6 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-50/50">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="w-10 h-10 rounded-xl text-red-600 flex items-center justify-center bg-[#F4F6FA] border border-slate-100">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 leading-none mb-1">S/
                        {{ number_format((float) ($d['expensesToday'] ?? 0), 2) }}</p>
                    <p class="text-[10px] font-bold text-slate-400">{{ $periodLabel }}</p>
                </div>
            </article>
            ...
            <article class="bg-white p-6 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-50/50">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="w-10 h-10 rounded-xl text-blue-500 flex items-center justify-center bg-[#F4F6FA] border border-slate-100">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 leading-none mb-1">
                        {{ number_format((int) ($d['ordersActive'] ?? 0)) }}</p>
            ...
            <article class="bg-white p-6 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-50/50">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="w-10 h-10 rounded-xl text-purple-500 flex items-center justify-center bg-[#F4F6FA] border border-slate-100">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 leading-none mb-1">
                        {{ number_format((int) ($d['maintenancesWeek'] ?? 0)) }}</p>
            ...
            <article class="bg-white p-6 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-50/50">
                <div class="flex justify-between items-start mb-6">
                    <div
                        class="w-10 h-10 rounded-xl text-amber-500 flex items-center justify-center bg-[#FFF7ED] border border-amber-100">
                ...
                <div>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Produccion del periodo
                    </p>
                    <p class="text-3xl font-black text-slate-900 leading-none mb-1">S/
                        {{ number_format((float) ($d['productionAmount'] ?? 0), 2) }}</p>
                    <p class="text-[10px] font-bold text-slate-400">{{ $periodLabel }}</p>
                </div>
            </article>
        </section>
        ...
        <section class="grid grid-cols-1 gap-6 xl:grid-cols-12 mb-6 main-trend-grid print:!block">
            <article
                class="bg-white rounded-[1.5rem] border border-slate-100 p-4 sm:p-6 xl:col-span-8 flex flex-col relative overflow-hidden transition-all hover:shadow-md trend-chart-article"
                x-data="{
                ...
                <!-- Header del gráfico -->
                <div
                    class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5 sm:mb-6 relative z-10">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-lg bg-[#F4F6FA] border border-slate-100 flex items-center justify-center shadow-sm">
                    ...
                    <div class="text-right bg-[#F4F6FA] px-4 py-3 rounded-xl border border-slate-100 w-full sm:w-auto">
                        ...
                        <p class="text-2xl font-bold text-blue-700 leading-none">S/
                            {{ number_format((float) ($incomeByDay->sum('amount')), 2) }}</p>
                ...
                <div class="flex-1 flex flex-col min-h-[240px] sm:min-h-[300px]">
                    <div class="flex flex-1 items-stretch gap-2 sm:gap-4">
                        ...
                        <!-- Eje Y (Labels) -->
                        <div
                            class="hidden sm:flex flex-col justify-between text-[11px] text-slate-400 font-medium pr-2 mb-6 mt-1">
                        ...
                        <div class="relative flex-1 h-[220px] sm:h-[260px] mb-3 sm:mb-4">
                            ...
                            <div class="absolute inset-0 h-full">
                                <svg viewBox="0 0 1000 100" preserveAspectRatio="none"
                                    ...
                                    <line x-show="activePoint" :x1="activePoint.x" :x2="activePoint.x" y1="0" y2="100"
                                        stroke="#93c5fd" stroke-width="1" stroke-dasharray="3 3"></line>
                                    @foreach($chartPoints as $pointIndex => $point)
                                        <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}"
                                            :r="activeIndex === {{ $pointIndex }} ? 4.5 : 3"
                                            :fill="activeIndex === {{ $pointIndex }} ? '#1d4ed8' : '#2563eb'" stroke="white"
                                            stroke-width="2" class="cursor-pointer transition-all duration-150"
            ...
            <!-- CUMPLEAÑOS -->
            <article
                class="rounded-[1.5rem] border border-slate-100 bg-white p-6 xl:col-span-4 h-full flex flex-col overflow-hidden print:!hidden">
                <div class="mb-8 flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-orange-600 text-white flex items-center justify-center shadow-lg shadow-orange-600/20">
                ...
                <div class="space-y-4 overflow-y-auto pr-1 custom-scrollbar flex-1">
                    @forelse($birthdays as $bday)
                        <div
                            class="flex items-center justify-between p-4 rounded-2xl border border-transparent birthday-hover-row cursor-pointer transition-all group/item">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-full text-white flex items-center justify-center font-black text-sm shadow-lg shadow-blue-500/20"
                                    style="background-color: #2563eb !important;">
                                    {{ substr($bday->first_name, 0, 1) }}{{ substr($bday->last_name, 0, 1) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h4
                                        class="font-black text-slate-800 uppercase text-sm leading-tight mb-1 whitespace-normal break-words">
                                        {{ $bday->first_name }} {{ $bday->last_name }}</h4>
                            ...
                            <a href="https://wa.me/{{ preg_replace('/\D/', '', $bday->phone) }}" target="_blank"
                                class="w-11 h-11 rounded-[1.2rem] text-white flex items-center justify-center hover:scale-110 transition-all shrink-0 shadow-lg shadow-emerald-500/20"
        ...
        <div class="hidden print:block space-y-12">
            ...
            <section class="break-inside-avoid">
                <div class="mb-4 flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl text-emerald-600 bg-emerald-50 flex items-center justify-center border border-emerald-100">
                    ...
                    <table class="w-full text-left">
                            ...
                            @forelse($salesTodayDetails as $sale)
                                <tr>
                                    <td class="px-6 py-4 text-xs font-bold text-slate-500">
                                        {{ \Carbon\Carbon::parse($sale->created_at)->format('H:i') }}</td>
                                    ...
                                    <td class="px-6 py-4 text-xs font-bold text-slate-700 uppercase">
                                        {{ $sale->client->first_name ?? '' }} {{ $sale->client->last_name ?? '' }}</td>
                                    <td class="px-6 py-4 text-right text-xs font-black text-slate-900">S/
                                        {{ number_format($sale->total, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 text-center text-xs text-slate-400 italic font-medium">Sin
                                        movimientos hoy</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot class="bg-emerald-50/50">
                            <tr class="font-black text-emerald-700">
                                <td colspan="3" class="px-6 py-4 text-xs text-right uppercase tracking-widest">Total
                                    Ingresado:</td>
                                <td class="px-6 py-4 text-right text-sm">S/
                                    {{ number_format($salesTodayDetails->sum('total'), 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <section class="break-inside-avoid">
                <div class="mb-4 flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl text-red-600 bg-red-50 flex items-center justify-center border border-red-100">
                        <i class="ri-money-dollar-circle-fill text-xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-red-600 uppercase tracking-widest leading-none mb-1">DETALLE
                            OPERATIVO</p>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 border-b-2 border-slate-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Descripción /
                                    Proveedor</th>
                                <th class="px-6 py-4 text-right text-[10px] font-black uppercase text-slate-400">Monto</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($expensesTodayDetails as $exp)
                                <tr>
                                    <td class="px-6 py-4 text-[10px] font-black uppercase"><span
                                            class="bg-gray-100 px-2 py-0.5 rounded">{{ $exp->type }}</span></td>
                                    <td class="px-6 py-4 text-xs font-bold text-slate-500">{{ $exp->reference ?: '-' }}</td>
                                    <td class="px-6 py-4 text-xs font-bold text-slate-700 uppercase">
                                        {{ $exp->description ?: ($exp->provider->legal_name ?? 'S/P') }}</td>
                                    <td class="px-6 py-4 text-right text-xs font-black text-red-600">S/
                                        {{ number_format($exp->total, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="py-10 text-center text-xs text-slate-400 italic font-medium">Sin
                                        egresos registrados hoy</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot class="bg-red-50/50">
                            <tr class="font-black text-red-700">
                                <td colspan="3" class="px-6 py-4 text-xs text-right uppercase tracking-widest">Total
                                    Egresado:</td>
                                <td class="px-6 py-4 text-right text-sm">S/
                                    {{ number_format($expensesTodayDetails->sum('total'), 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <section class="break-inside-avoid">
                <div class="mb-4 flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl text-blue-600 bg-blue-50 flex items-center justify-center border border-blue-100">
                        <i class="ri-folder-open-fill text-xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-blue-600 uppercase tracking-widest leading-none mb-1">DATOS DE
                            TALLER</p>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 border-b-2 border-slate-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Cliente / Vehículo
                                </th>
                                <th class="px-6 py-4 text-[10px] font-black uppercase text-slate-400">Estado</th>
                                <th class="px-6 py-4 text-right text-[10px] font-black uppercase text-slate-400">Ingresó
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($activeOrdersDetails as $order)
                                <tr>
                                    <td class="px-6 py-4 text-xs font-black text-slate-900 text-center">
                                        {{ $order->movement->number ?? sprintf("%08d", $order->id) }}</td>
                                    <td class="px-6 py-4">
                                        <p class="text-xs font-black text-slate-800 uppercase leading-none mb-1">
                                        ...
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight">
                                            ...
                                            {{ ($order->vehicle->model ?? '') }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest bg-blue-50 text-blue-600 border border-blue-100">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-10 text-center text-xs text-slate-400 italic">No hay órdenes
                                        activas</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <section class="break-inside-avoid">
                <div class="mb-4 flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl text-purple-600 bg-purple-50 flex items-center justify-center border border-purple-100">
                        <i class="ri-service-fill text-xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-purple-600 uppercase tracking-widest leading-none mb-1">DATOS
                            DE TALLER</p>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 border-b-2 border-slate-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black uppercase text-slate-400 text-center">Servicios
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($maintenanceWeekDetails as $m)
                                <tr>
                                    <td class="px-6 py-4 text-[11px] font-bold text-slate-500 whitespace-nowrap">
                                        {{ \Carbon\Carbon::parse($m->finished_at ?? $m->delivery_date)->format('d/m/Y') }}</td>
                                    <td class="px-6 py-4">
                                        <p class="text-xs font-black text-slate-900 uppercase tracking-tighter">
                                            {{ $m->vehicle->plate ?? '-' }}</p>
                                        <p class="text-[9px] font-bold text-slate-400 uppercase truncate max-w-[150px]">
                                            {{ $m->client->first_name ?? '' }} {{ $m->client->last_name ?? '' }}</p>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] font-black">{{ $m->details_count ?? 0 }}
                                            ítems</span>
                                    </td>
                                    <td class="px-6 py-4 text-right text-xs font-black text-slate-900">S/
                                        {{ number_format($m->total, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="py-10 text-center text-xs text-slate-400 italic">Sin mantenimientos
                                        esta semana</td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <section class="grid grid-cols-1 gap-6 lg:grid-cols-2" x-data="{ 
                        showTechModal: false, 
                        loadingTech: false, 
                        techName: '', 
                        techServices: [],
            ...
            <!-- PRODUCTIVITY -->
            <article
                class="rounded-[1.5rem] border border-slate-100 bg-white p-6 flex flex-col h-full overflow-hidden relative">
                <div class="mb-8 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl text-white flex items-center justify-center shadow-lg"
                        style="background-color: #9333ea !important;">
                        <i class="ri-time-fill text-xl"></i>
                    </div>
                    <div>
                        <p class="text-[9px] font-black text-purple-600 uppercase tracking-widest leading-none mb-1">
                            RENDIMIENTO OPERATIVO</p>
                ...
                <div class="space-y-3 flex-1 pr-1 custom-scrollbar overflow-y-auto">
                    @forelse($techProductivity as $tech)
                        <div
                            class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-slate-100 transition-all">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full text-white flex items-center justify-center font-black text-xs shadow-md"
                                    style="background-color: #9333ea !important;">
                                    {{ substr($tech->technician, 0, 1) }}
                                </div>
                                <div>
                                    <p class="text-sm font-black text-slate-800 whitespace-normal break-words">
                                        {{ $tech->technician }}</p>
                                    <div class="flex items-center gap-2">
                                        <p class="text-[10px] text-slate-400 font-bold uppercase">Tiempo promedio:
                                            {{ number_format($tech->avg_minutes ?? 0, 0) }} min</p>
                                        <button type="button"
                ...
                <div x-show="showTechModal"
                    ...
                    <div @click.away="showTechModal = false"
                        ...
                        <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center shadow-lg">
                                    <i class="ri-user-settings-line text-xl"></i>
                                </div>
                                <div>
                                    <h4 class="text-lg font-black text-slate-800 leading-none mb-1" x-text="techName"></h4>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Historial de
                                        Productividad</p>
                                </div>
                            </div>
                            <button @click="showTechModal = false"
                                class="w-8 h-8 rounded-full hover:bg-slate-200 flex items-center justify-center transition-colors">
                        ...
                        <div class="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            ...
                            <template x-if="!loadingTech && techServices.length > 0">
                                <div
                                    class="overflow-x-auto overflow-y-hidden rounded-2xl border border-slate-100 shadow-sm custom-scrollbar">
                                    <table class="w-full min-w-[640px] text-left border-collapse">
                                        <thead>
                                            <tr class="bg-slate-50 border-b border-slate-100">
                                                <th
                                                    class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                                                    Orden</th>
                                                <th
                                                    class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                                                    Vehículo</th>
                                                <th
                                                    class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">
                                                    Inicio</th>
                                                <th
                                                    class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">
                                                    Fin</th>
                                            ...
                                            <template x-for="service in techServices" :key="service.os">
                                                <tr class="hover:bg-slate-50/50 transition-colors">
                                                    <td class="px-4 py-3 text-xs font-black text-slate-900"
                                                        x-text="service.os"></td>
                                                    <td class="px-4 py-3">
                                                        <p class="text-xs font-bold text-slate-700 truncate max-w-[180px]"
                                                            x-text="service.vehicle"></p>
                                                    </td>
                                                    <td class="px-4 py-3 text-center text-[10px] font-bold text-slate-500"
                                                        x-text="service.started_at"></td>
                                                    <td class="px-4 py-3 text-center text-[10px] font-bold text-slate-500"
                                                        x-text="service.finished_at"></td>
                                                    <td class="px-4 py-3 text-center">
                                                        <span
                                                            class="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-black"
                                                            x-text="service.net_minutes + ' min'"></span>
                                                    </td>
                                                    <td class="px-4 py-3 text-center">
                                                        <span
                                                            class="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-black"
                                                            x-text="service.paused_minutes + ' min'"></span>
                                                    </td>
                                                </tr>
                                            </template>
                                        </tbody>
                        ...
                        <div class="p-6 bg-slate-50/30 border-t border-slate-50 flex justify-end">
                            <button @click="showTechModal = false"
                                class="px-4 py-2 bg-slate-800 text-white rounded-xl text-xs font-black hover:bg-slate-900 transition-all shadow-lg">
                                Cerrar Detalle
                            </button>
                        </div>
                    </div>
            ...
            <!-- FREQUENT CLIENTS -->
            <article x-data="{ searchFrequentClients: '' }"
                class="rounded-[1.5rem] border border-slate-100 bg-white p-6 flex flex-col h-full overflow-hidden">
                <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[9px] font-black text-blue-600 uppercase tracking-widest leading-none mb-1">
                    ...
                    <div class="relative w-full sm:w-auto group">
                        <i
                            class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm group-focus-within:text-blue-500 transition-colors"></i>
                        <input type="text" x-model="searchFrequentClients" placeholder="Buscar cliente..."
                            class="pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:border-blue-500 focus:bg-white transition-all w-full md:w-48 placeholder:text-slate-300">
                    </div>
                </div>
                ...
                <div class="space-y-3 flex-1 pr-1 custom-scrollbar overflow-y-auto">
                    @forelse($frequentClients as $client)
                        <div class="group flex items-center gap-4 p-4 rounded-2xl bg-blue-50 border border-transparent client-hover-row transition-all"
                            x-show="searchFrequentClients.trim() === '' ? {{ $loop->iteration }} <= 5 : '{{ addslashes(strtolower($client->client)) }}'.includes(searchFrequentClients.toLowerCase())">
                            <div class="w-10 h-10 rounded-full text-white flex items-center justify-center font-black text-xs shrink-0 shadow-md"
                                style="background-color: #2563eb !important;">
                                {{ $loop->iteration }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p
                                    class="text-[13px] font-black text-slate-800 whitespace-normal break-words uppercase tracking-tight">
                                    {{ $client->client }}</p>
                            ...
                            <button
                                class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 transition-all shrink-0 shadow-lg print:hidden">
        ...
        <section class="grid grid-cols-1 gap-6 xl:grid-cols-12">
            <!-- TOP SERVICES -->
            <article x-data="{ searchTopServices: '' }"
                class="rounded-[1.5rem] border border-slate-100 bg-white p-6 xl:col-span-4 h-full flex flex-col overflow-hidden print-hidden">
                <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl text-white flex items-center justify-center shadow-lg"
                        ...
                        <div>
                            <p class="text-[9px] font-black text-emerald-600 uppercase tracking-widest leading-none mb-1">LO
                                MÁS SOLICITADO</p>
                    ...
                    <div class="relative w-full sm:w-auto group">
                        <i
                            class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm group-focus-within:text-emerald-500 transition-colors"></i>
                        <input type="text" x-model="searchTopServices" placeholder="Buscar servicio..."
                            class="pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:border-emerald-500 focus:bg-white transition-all w-full md:w-48 placeholder:text-slate-300">
                    </div>
                </div>
                ...
                <div class="space-y-6 flex-1 pr-1 overflow-y-auto custom-scrollbar">
                    @forelse($topServices as $service)
                        <div class="block"
                            x-show="searchTopServices.trim() === '' ? {{ $loop->iteration }} <= 5 : '{{ addslashes(strtolower($service->name)) }}'.includes(searchTopServices.toLowerCase())">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl text-white flex items-center justify-center font-black text-xs shadow-md"
                                        style="background-color: #10b981 !important;">
                                        {{ $loop->iteration }}
                                    </div>
                                    <div>
                                        <p class="text-[13px] font-black text-slate-800 leading-none mb-1">{{ $service->name }}
                                        </p>
                                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">
                                            {{ number_format($service->qty, 0) }} ATENCIONES</p>
                                    </div>
                                </div>
                                <p class="text-sm font-black text-slate-900">S/ {{ number_format($service->amount, 0) }}</p>
                            </div>
                            <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-full bg-emerald-500 rounded-full shadow-sm"
                                    style="width: {{ min(100, (int) ($service->qty * 5)) }}%"></div>
                            </div>
                        </div>
                    @empty
            ...
            <article x-data="{
                ...
                <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 print:hidden">
                        <div class="relative group">
                            <i
                                class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm group-focus-within:text-[#465fff] transition-colors"></i>
                            <input type="text" x-model="search" placeholder="Buscar..."
                                class="pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:border-[#465fff] focus:bg-white transition-all w-full md:w-56 placeholder:text-slate-300">
                        </div>
                        <div class="relative" @click.away="showStatusMenu = false">
                            <button type="button" @click="showStatusMenu = !showStatusMenu"
                                class="p-2.5 border rounded-xl transition-all" :class="showStatusMenu || selectedStatus !== 'all'
                                        ? 'bg-blue-50 border-blue-200 text-blue-600'
                                        : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100'">
                                <i class="ri-filter-3-line"></i>
                            </button>
                            <div x-show="showStatusMenu" x-transition
                                class="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-lg z-20 overflow-hidden"
                                x-cloak>
                                <div class="px-3 py-2 border-b border-slate-100">
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Filtrar por
                                        estado</p>
                                </div>
                                <div class="max-h-64 overflow-y-auto custom-scrollbar">
                                    <template x-for="option in statusOptions" :key="option.value">
                                        <button type="button" @click="setStatus(option.value)"
                                            class="w-full px-3 py-2.5 text-left text-xs font-bold transition-colors flex items-center justify-between"
                ...
                <div class="mb-4 print:hidden" x-show="selectedStatus !== 'all'">
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-wider border border-blue-100">
                        <i class="ri-filter-3-line"></i>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <template x-for="order in filteredOrders" :key="order.id">
                                <tr class="group hover:bg-slate-50 transition-all duration-200">
                                    <td class="px-6 py-4">
                                        <span
                                            class="text-[11px] font-black text-slate-400 group-hover:text-slate-900 transition-colors"
                                            x-text="order.number"></span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p class="text-[13px] font-black text-slate-800 uppercase leading-tight"
                                            x-text="order.client"></p>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-0.5"
                                            x-text="order.vehicle"></p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                    ...
                                    <td class="px-6 py-4 text-center text-[11px] font-bold text-slate-500"
                                        x-text="order.date"></td>
                                    <td class="px-6 py-4 text-right">
                                        <span class="text-[13px] font-black"
                                            :class="order.pending > 0 ? 'text-slate-900' : 'text-slate-300'"
                                            x-text="'S/ ' + parseFloat(order.pending).toFixed(2)"></span>
                                    </td>
                                </tr>
                            </template>
                            <template x-if="filteredOrders.length === 0">
                                <tr>
                                    <td colspan="5" class="py-16 text-center text-sm text-slate-300 italic">No hay órdenes
                                        que coincidan con la búsqueda.</td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
