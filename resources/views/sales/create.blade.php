...
$primaryActionLabel =
$invoiceMode && $isEditMode ? 'Guardar factura' : ($isEditMode ? 'Guardar cambios' : 'Cobrar');
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="grid grid-cols-1 gap-3 xl:grid-cols-12 xl:items-end">
                        <div class="xl:col-span-4">
                            <label for="sale-moved-at"
                                class="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Fecha
                                de venta</label>
                        ...
                        <div class="xl:col-span-4">
                            <label for="document-type-select"
                                class="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Documento
                                <span class="text-red-500">*</span></label>
                            <x-form.select-autocomplete name="" selectId="document-type-select"
                                :value="(string) ((int) ($defaultDocumentTypeId ?? 0))" :options="collect($documentTypes ?? [])
                                        ->map(
                                            fn($documentType) => [
                                        ...
                                        ->values()
                                        ->all()" placeholder="Seleccione documento"
                        ...
                        <div class="xl:col-span-2">
                            <label for="sale-header-series"
                                class="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Serie</label>
                        ...
                        <div class="xl:col-span-2">
                            <label for="sale-header-number"
                                class="mb-2 block text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Numero
                                <span class="text-red-500">*</span></label>
                    ...
                    <div id="summary-tab-panel">
                        <div class="border-b border-slate-200 bg-slate-50 p-4">
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <label for="sale-detail-type-select"
                                    class="mb-2 block text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Tipo
                                    detalle</label>
                                <select id="sale-detail-type-select"
                                ...
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <div class="space-y-2">
                                        <label for="payment-type-select"
                                            class="block text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Tipo
                                            pago</label>
                                        <x-form.select-autocomplete name="" selectId="payment-type-select"
                                            value="CONTADO" :options="[
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <x-form.select-autocomplete name="" selectId="cash-register-select"
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label for="sale-debt-credit-days"
                                                class="block text-[10px] font-bold uppercase tracking-[0.18em] text-amber-800">Días
                                                de crédito</label>
                                        ...
                                        <div class="space-y-2">
                                            <label for="sale-debt-due-date"
                                                class="block text-[10px] font-bold uppercase tracking-[0.18em] text-amber-800">Fecha
                                                vencimiento</label>
                                ...
                                <div id="invoice-billing-block"
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-3">
                                        <div class="space-y-2">
                                            ...
                                            <x-form.select-autocomplete name="" selectId="billing-status-select"
                            ...
                            <div id="payment-methods-section">
                                ...
                                <div class="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
                                    ...
                                    <div id="payment-difference-wrap"
                                        ...
                                        <span id="payment-difference" class="font-black"
                                            style="color:#ea580c;">$0.00</span>
                                    </div>
                                </div>
                            </div>
                            <div class="mt-4">
...
@php
$peopleJson = ($people ?? collect())->map(function ($person) {
...
<script>
    (function() {
            const products = Array.isArray(@json($products ?? [])) ? @json($products ?? []) : Object.values(
                @json($products ?? []) || {});
            const productBranches = Array.isArray(@json($productBranches ?? ($productsBranches ?? []))) ? @json($productBranches ?? ($productsBranches ?? [])) : Object
                .values(@json($productBranches ?? ($productsBranches ?? [])) || {});
            let people = @json($peopleJson);
            ...
            const paymentMethods = Array.isArray(@json($paymentMethods ?? [])) ?
                @json($paymentMethods ?? []) : [];
            const paymentGateways = Array.isArray(@json($paymentGateways ?? [])) ?
            ...
            let selectedDetailType = String(initialSaleData?.detail_type || 'DETALLADO').toUpperCase() ===
                'GLOSA' ?
                'GLOSA' :
                'DETALLADO';
            ...
            const defaultClient = people.find((person) => Number(person.id) === Number(defaultClientId)) ||
                null;
            let currentSale = isEditMode && initialSaleData ? {
                    id: Number(initialSaleData.id || 0) || null,
                    number: String(initialSaleData.number || ''),
                    clientId: initialSaleData.clientId ? Number(initialSaleData.clientId) : (defaultClient ?
                        defaultClient.id : null),
                    clientName: initialSaleData.clientName || (defaultClient ? defaultClient.label :
                        'Publico General'),
                    status: 'editing',
                    ...
                    items: Array.isArray(initialSaleData.items) ?
                        ...
                        .map((item) => ({
                            kind: String(item.kind || (Number(item.pId || 0) > 0 ? 'product' :
                                'glosa')),
                    ...
                    payment_methods: Array.isArray(initialSaleData.payment_methods) ? initialSaleData
                        .payment_methods : [],
                    payment_type: String(initialSaleData.payment_type || 'CONTADO'),
                    document_type_id: Number(initialSaleData.document_type_id || defaultDocumentTypeId || 0) ||
                        null,
                    cash_register_id: Number(initialSaleData.cash_register_id || defaultCashRegisterId || 0) ||
                        null,
                    detail_type: String(initialSaleData.detail_type || selectedDetailType || 'DETALLADO')
                        .toUpperCase() === 'GLOSA' ? 'GLOSA' : 'DETALLADO',
            ...
            currentSale.document_type_id = Number(currentSale.document_type_id ||
                defaultDocumentTypeId || 0) || null;
            currentSale.cash_register_id = Number(currentSale
                .cash_register_id || defaultCashRegisterId || 0) || null;
            currentSale.detail_type = String(
                    currentSale.detail_type || selectedDetailType || 'DETALLADO')
                .toUpperCase() === 'GLOSA' ? 'GLOSA' : 'DETALLADO';
            selectedDetailType = currentSale
                .detail_type;
            currentSale.payment_type = String(currentSale.payment_type || 'CONTADO')
                .toUpperCase() === 'DEUDA' ?
                'DEUDA' : 'CONTADO';
            currentSale.billing_status = String(currentSale.billing_status ||
                'NOT_APPLICABLE');
            currentSale.invoice_series = String(currentSale.invoice_series ||
                '001');
            ...
            currentSale.debt_due_date =
                String(currentSale.debt_due_date || '').trim();
            paymentRows = Array.isArray(currentSale
                    .payment_methods) ?
                currentSale.payment_methods.map((row) => ({
                    payment_method_id: Number(row.payment_method_id || row.methodId || paymentMethods[0]
                        ?.id ||
                        0),
            ...
            if (invoiceMode && invoiceDocumentIds.has(Number(currentSale.document_type_id || 0)) && currentSale
            ...
            const saleHeaderPreviewUrl =
                @json(route('admin.sales.preview.header'));
            ...
            const renderQuickClientLocationOptions = () => {
                ...
                renderSelectOptions(department, departments, selectedDepartmentId,
                    'Seleccione departamento');
                renderSelectOptions(
                    province,
                    provinces.filter((item) => String(item.parent_location_id || '') ===
                ...
                renderSelectOptions(
                    district,
                    districts.filter((item) => String(item.parent_location_id || '') ===
                        selectedProvinceId),
                    selectedDistrictId,
                    'Seleccione distrito'
                );
            ...
            const upsertClientInList = (payload) => {
                const client = {
                    ...
                    label: String(payload.label || payload.name ||
                        `${payload.first_name || ''} ${payload.last_name || ''}`).trim() || String(
                        payload
                        .document || 'Sin nombre'),
            ...
            const fetchQuickClientDocumentData = async () => {
                ...
                if (type === 'DNI') {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            `${reniecApiUrl}?dni=${encodeURIComponent(documentValue)}`, {
                                headers: {
                                    Accept: 'application/json'
                                },
                            });
                        ...
                        const lnUnified = String(payload?.last_name ?? '').trim() || [apPat, apMat]
                            .filter(
                                Boolean).join(' ');
                        ...
                        if (firstName) firstName.value = String(fnVal || parsed.first_name || '')
                            .trim();
                        if (lastName) lastName.value = String(lnUnified || parsed.last_name || '')
                            .trim();
                ...
                if (type === 'RUC') {
                    ...
                    try {
                        toggleQuickClientLoading(true);
                        const response = await fetch(
                            `${rucApiUrl}?ruc=${encodeURIComponent(documentValue)}`, {
                                headers: {
                                    Accept: 'application/json'
                                },
                            });
                        ...
                        if (date) date.value = normalizeApiDate(payload?.raw?.fecha_inscripcion ||
                            payload?.raw
                            ?.fechaInscripcion || '');
            ...
            const saveQuickClient = async () => {
                ...
                if (!payload.document_number || !payload.first_name || (!payload.last_name && payload
                        .person_type !== 'RUC')) {
            ...
            const isInvoiceDocumentSelected = () => invoiceDocumentIds.has(Number(currentSale
                .document_type_id || 0));
            const preferredCashRegisterIdForCurrentDocument = () => {
                const preferredId = isInvoiceDocumentSelected() ? invoiceCashRegisterId :
            ...
            const handlePaymentTypeChange = (nextValue) => {
                currentSale.payment_type = String(nextValue || 'CONTADO').toUpperCase() === 'DEUDA' ?
                    'DEUDA' :
                    'CONTADO';
                if (isDebtSaleSelected()) {
                    paymentRows = [];
            ...
            const getProductCategory = (prod) => (prod && prod.category && String(prod.category).trim() !==
                    '') ?
                String(prod.category).trim() : 'Sin categoria';
            ...
            const bindSalePaymentMethodAcOutsideClose = () => {
                ...
                document.addEventListener('click', (e) => {
                    if (!e.target.closest('.js-payment-method-ac')) {
                        document.querySelectorAll('#payment-rows .payment-method-ac-dropdown')
                            .forEach((
                ...
                root.querySelectorAll('.js-payment-method-ac').forEach((wrap) => {
                    ...
                    const renderList = (filter) => {
                        ...
                        paymentMethodVariants.forEach((variant) => {
                            if (q && !String(variant.label || '').toLowerCase()
                                .includes(q))
                                return;
                            count += 1;
                            ...
                            btn.type = 'button';
                            btn.className =
                                'flex w-full px-3 py-2.5 text-left text-sm text-slate-800 hover:bg-orange-50';
                    ...
                    trigger.addEventListener('click', (ev) => {
                        ...
                        document.querySelectorAll(
                                '#payment-rows .payment-method-ac-dropdown')
            ...
            function renderClientOptions() {
                ...
                if (clearButton) {
                    clearButton.classList.toggle('hidden', !currentSale.clientId && !(clientQuery || '')
                        .trim());
                }
            ...
            function selectClient(client) {
                ...
                currentSale.clientName = client ? client.label : (defaultClient ? defaultClient.label :
                    'Publico General');
                clientQuery = client ? (client.document ? `${client.label} - ${client.document}` : client
                    .label) : '';
                saveDB();
            ...
            function setAsideTab(tab) {
                ...
                if (summaryButton) {
                    ...
                    summaryButton.style.background = currentAsideTab === 'summary' ?
                        'linear-gradient(90deg,#ff7a00,#ff4d00)' : '';
                    summaryButton.style.color = currentAsideTab === 'summary' ? '#fff' : '';
                    summaryButton.style.boxShadow = currentAsideTab === 'summary' ?
                ...
                if (paymentButton) {
                    paymentButton.className = currentAsideTab === 'payment' ?
                        'inline-flex h-9 items-center justify-center rounded-lg text-sm font-bold text-white shadow-theme-xs' :
                        'inline-flex h-9 items-center justify-center rounded-lg text-sm font-bold text-slate-300 transition-colors hover:text-white';
                    paymentButton.style.background = currentAsideTab === 'payment' ?
                        'linear-gradient(90deg,#ff7a00,#ff4d00)' : '';
                    paymentButton.style.color = currentAsideTab === 'payment' ? '#fff' : '';
                    paymentButton.style.boxShadow = currentAsideTab === 'payment' ?
                        '0 8px 18px rgba(249,115,22,0.24)' :
                        '';
                }
            }
            ...
            function addPaymentRow(prefillAmount = null) {
                ...
                paymentRows.push({
                    ...
                    amount: prefillAmount != null ? Number(prefillAmount) : (paymentRows.length === 0 ?
                        total :
                        remaining),
            ...
            function renderCategoryFilters() {
                ...
                getCategories().forEach((category) => {
                    ...
                    button.type = 'button';
                    button.className =
                        'inline-flex min-h-[44px] items-center justify-center rounded-[22px] border px-4 py-2 text-sm font-bold transition sm:px-5';
            ...
            function defaultGlosaUnitId() {
                const exact = units.find((unit) => String(unit.description || '').trim().toLowerCase() ===
                    'unidad(es)');
                if (exact) return Number(exact.id || 0) || null;
                const startsUnidad = units.find((unit) => /^unidad/i.test(String(unit.description || '')
                    .trim()));
            ...
            function renderGlosaPanel() {
                ...
                glosaItems.forEach((item, index) => {
                    ...
                    row.querySelector('[data-role="glosa-name"]')?.addEventListener('input', (
                        event) => {
                        currentSale.items[index].name = String(event.currentTarget.value || '');
                        saveDB();
                        renderTicket();
                    });
                    row.querySelector('[data-role="glosa-unit"]')?.addEventListener('change', (
                        event) => {
                        currentSale.items[index].unit_id = Number(event.currentTarget.value ||
                                0) ||
                            null;
                        saveDB();
                    });
                    row.querySelector('[data-role="glosa-qty"]')?.addEventListener('input', (event) => {
                        currentSale.items[index].qty = Math.max(0.0001, Number(event
                            .currentTarget
                            .value || 0) || 1);
                        saveDB();
                        renderTicket();
                    });
                    row.querySelector('[data-role="glosa-amount"]')?.addEventListener('input', (
                        event) => {
                        currentSale.items[index].price = sanitizeLineMoney(event.currentTarget
                            .value);
                        saveDB();
                        renderTicket();
                    });
                    row.querySelector('[data-role="glosa-note"]')?.addEventListener('input', (
                        event) => {
            ...
            function renderProducts() {
                ...
                products.forEach((prod) => {
                    ...
                    const searchNeedle = `${prod.code || ''} ${prod.name || ''} ${category}`
                        .toLowerCase();
                    ...
                    card.type = 'button';
                    card.className =
                        'group relative overflow-hidden border bg-white text-center transition-all duration-200';
                    ...
                    card.addEventListener('mouseenter', () => {
                        ...
                        if (orb) {
                            ...
                            orb.style.boxShadow =
                                '0 18px 30px rgba(249, 115, 22, 0.12), 0 8px 16px rgba(15, 23, 42, 0.06)';
                        }
                    });
                    card.addEventListener('mouseleave', () => {
                        ...
                        if (orb) {
                            orb.style.transform = '';
                            orb.style.boxShadow =
                                '0 12px 24px rgba(249, 115, 22, 0.08), 0 6px 14px rgba(15, 23, 42, 0.04)';
                        }
                    });
                ...
                if (rendered === 0) {
                    grid.innerHTML =
                        '<div class="col-span-full rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-sm text-slate-500">No se encontraron productos para el filtro actual.</div>';
                }
            }
            ...
            function renderTicket() {
                ...
                if (!currentSale.items.length) {
                    ...
                    currentSale.items.forEach((item, index) => {
                        ...
                        const row = document.createElement('div');
                        row.className =
                            'mb-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm';
                        row.innerHTML = `
                                <div class="p-3">
                            ...
                            `;
                        row.querySelector('[data-role="unit-price"]')?.addEventListener('change', (
                            event) => {
                            setItemUnitPrice(index, event.currentTarget.value);
                        });
                        row.querySelector('[data-role="line-name"]')?.addEventListener('input', (
                            event) => {
                            if (!currentSale.items[index]) return;
                            currentSale.items[index].name = String(event.currentTarget.value ||
                                '');
                        ...
                        row.querySelector('[data-role="line-total"]')?.addEventListener('change', (
                            event) => {
            ...
            function processSaleNow() {
                ...
                const invalidCardRow = isDebtSaleSelected() ? null : paymentRows.find((row) => {
                    return isCardMethod(row.payment_method_id) && (!row.payment_gateway_id || !row
                        .card_id);
                });
                if (invalidCardRow) {
                ...
                const payload = {
                    items: currentSale.items.filter((item) => Number(item.pId || 0) > 0 || String(item
                            .name || '')
                        .trim() !== '').map((item) => ({
                        kind: String(item.kind || (Number(item.pId || 0) > 0 ? 'product' :
                            'glosa')),
                    ...
                    cash_register_id: Number(currentSale.cash_register_id || document.getElementById(
                        'cash-register-select')?.value || 0),
                    ...
                    ...(isDebtSaleSelected() ? {
                        credit_days: Math.max(0, parseInt(currentSale.credit_days, 10) || 0),
                        debt_due_date: String(currentSale.debt_due_date || document.getElementById(
                            'sale-debt-due-date')?.value || '').trim() || null,
                    } : {}),
                };
            ...
            function goBack() {
                ...
                fetch(@json(route('admin.sales.draft')), {
                    ...
                    body: JSON.stringify({
                        detail_type: selectedDetailType,
                        items: currentSale.items.filter((item) => Number(item.pId || 0) > 0 ||
                            String(
                                item.name || '').trim() !== '').map((item) => ({
                            kind: String(item.kind || (Number(item.pId || 0) > 0 ?
                                'product' : 'glosa')),
                            pId: Number(item.pId || 0) || null,
                            name: String(item.name || '').trim(),
                            qty: Number(item.qty),
                        ...
                        payment_type: currentSale.payment_type || 'CONTADO',
                        document_type_id: Number(document.getElementById('document-type-select')
                            ?.value || 0) || null,
                        billing_status: isInvoiceDocumentSelected() ? currentSale
                            .billing_status : 'NOT_APPLICABLE',
                        invoice_series: isInvoiceDocumentSelected() ? (currentSale
                            .invoice_series ||
                            '') : '',
                        invoice_number: isInvoiceDocumentSelected() && currentSale
                            .billing_status ===
                            'INVOICED' ? (currentSale.invoice_number || '') : '',
                        notes: document.getElementById('sale-notes')?.value ||
                            'Venta guardada como borrador - pendiente de pago'
                    })
                }).finally(() => {
                    window.location.href = @json($salesIndexUrl);
                });
            ...
            document.getElementById('sale-detail-type-select')?.addEventListener('change', (event) => {
                selectedDetailType = String(event.target.value || 'DETALLADO').toUpperCase() === 'GLOSA' ?
                    'GLOSA' : 'DETALLADO';
            ...
            document
                .getElementById('add-glosa-button')?.addEventListener('click', addGlosaLine);
            document
                .getElementById('add-payment-row-button')?.addEventListener('click', () =>
                    addPaymentRow());
            document.getElementById('summary-tab-button')?.addEventListener('click', () =>
                setAsideTab('summary'));
            document.getElementById('payment-tab-button')?.addEventListener(
                'click', () => setAsideTab('payment'));
            document.getElementById('sale-notes')?.addEventListener(
                'input', (event) => {
            ...
            document
                .getElementById('open-quick-client-modal-button')?.addEventListener('click',
                    openQuickClientModal);
            document.getElementById('quick-client-close-button')?.addEventListener(
                'click', closeQuickClientModal);
            document.getElementById('quick-client-cancel-button')
                ?.addEventListener('click', closeQuickClientModal);
            document.getElementById(
                'quick-client-modal-backdrop')?.addEventListener('click', closeQuickClientModal);
            document
                .getElementById('quick-client-search-button')?.addEventListener('click',
                    fetchQuickClientDocumentData);
            document.getElementById('quick-client-date-picker-btn')
            ...
            document.getElementById('quick-client-person-type')?.addEventListener('change',
                syncQuickClientPersonTypeUI);
            document.getElementById('quick-client-department')
                ?.addEventListener('change',
                    onQuickClientDepartmentChange);
            document.getElementById('quick-client-province')
                ?.addEventListener('change', onQuickClientProvinceChange);
            document.getElementById(
                'quick-client-form')?.addEventListener('submit', (event) => {
                event.preventDefault();
                saveQuickClient();
            });
            ...
            const clientInput = document.getElementById('client-autocomplete');
            if (clientInput) {
                clientInput.value = currentSale.clientName || (defaultClient ? defaultClient.label :
                    'Publico General');
            ...
            syncQuickClientPersonTypeUI();
            setQuickClientLocation(branchDepartmentId, branchProvinceId,
                branchDistrictId
            );
            renderDetailTypeSelect();
            renderCategoryFilters();
            ...
            const bootPosAlpineAutocompleteSelects = () => {
                ...
                const billingStatusSelect = document.getElementById('billing-status-select');
                if (billingStatusSelect) {
                    billingStatusSelect.value = ['INVOICED', 'PENDING'].includes(currentSale
                            .billing_status) ?
                        currentSale.billing_status : 'PENDING';
                    syncAutocompleteDisplay(billingStatusSelect);
                }
                syncPaymentTypeUI();
                syncInvoiceBillingFields();
