...
<x-ui.modal x-data="{ open: false, paymentConceptId: null, description: '', type: '' }"
    ...
    <div class="p-6 space-y-4">
        ...
        @endif
        <form id="edit-payment-concept-form" class="space-y-4"
            x-bind:action="paymentConceptId ? '{{ url('/admin/herramientas/conceptos-pago') }}/' + paymentConceptId + '{{ request('view_id') ? '?view_id=' . request('view_id') : '' }}' : '#'"
            method="POST">
            @csrf
            @method('PUT')
            @if (request('view_id'))
