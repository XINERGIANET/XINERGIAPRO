    ...
    <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-ui.link-button size="md" variant="outline" href="{{ $viewId ? route('admin.payment_concepts.index', ['view_id' => $viewId]) : route('admin.payment_concepts.index') }}" class="flex-1 sm:flex-none h-11 px-4 border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
        ...
        <div class="flex flex-wrap items-center gap-2">
            @foreach ($topOperations as $operation)
                @php
                    ...
                    $isCreate = str_contains($operation->action ?? '', 'payment_concepts.create')
                ...
                @if ($isCreate)
                    <x-ui.button size="md" variant="primary" type="button"
                        style="{{ $topStyle }}" @click="$dispatch('open-create-payment-concept-modal')">
        ...
        <div class="table-responsive mt-4 rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <thead class="text-left text-theme-xs dark:text-gray-400">
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    <th style="background-color: #334155; color: #FFFFFF;" class="px-5 py-3 text-center sm:px-6 sticky-left-header">ID</th>
                    <th style="background-color: #334155; color: #FFFFFF;" class="px-5 py-3 text-center sm:px-6">Descripcion</th>
                    <th style="background-color: #334155; color: #FFFFFF;" class="px-5 py-3 text-center sm:px-6">Tipo</th>
...
<x-ui.modal x-data="{ open: false }" @open-create-payment-concept-modal.window="open = true"
    ...
    <div class="p-6 space-y-4">
        ...
        @endif
        <form id="create-payment-concept-form" class="space-y-4" action="{{ $viewId ? route('admin.payment_concepts.store') . '?view_id=' . $viewId : route('admin.payment_concepts.store') }}"
            method="POST">
            @csrf
            @if ($viewId)
                <input type="hidden" name="view_id" value="{{ $viewId }}">
