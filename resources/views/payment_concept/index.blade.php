...
@section('content')
    @php
        ...
        $resolveActionUrl = function ($action, $model = null, $operation = null) use ($viewId) {
            ...
            $normalizedAction = str_replace('payment.concepts', 'payment_concepts', $action);
        ...
        $resolveTextColor = function ($operation) {
            ...
            if (str_contains($action, 'payment_concepts.create') || str_contains($action, 'payment.concepts.create')) {
                return '#111827';
            }
            return '#FFFFFF';
        };
    ...
    <x-common.component-card title="Listado de conceptos de pago"
        desc="Gestiona los conceptos de pago registrados en el sistema.">
            ...
            <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.link-button size="md" variant="outline"
                        href="{{ $viewId ? route('admin.payment_concepts.index', ['view_id' => $viewId]) : route('admin.payment_concepts.index') }}"
            ...
            <div class="flex flex-wrap items-center gap-2">
                @foreach ($topOperations as $operation)
                    @php
                        ...
                        $isCreate =
                            str_contains($operation->action ?? '', 'payment_concepts.create') ||
                            str_contains($operation->action ?? '', 'payment.concepts.create') ||
                            str_contains($operation->action ?? '', 'open-create-payment-concept-modal');
                    @endphp
                    @if ($isCreate)
                        <x-ui.button size="md" variant="primary" type="button" style="{{ $topStyle }}"
                            @click="$dispatch('open-create-payment-concept-modal')">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach ($paymentConcepts as $paymentConcept)
                            <tr class="border-b border-gray-100 dark:border-gray-800 dark:hover:bg-white/5">
                                <td
                                    class="px-5 py-4 sm:px-6 text-center font-medium text-gray-900 text-theme-sm dark:text-white/90 sticky-left">
                                    {{ $paymentConcept->id }}</td>
                                <td
                                    class="px-5 py-4 sm:px-6 text-center font-medium text-gray-900 text-theme-sm dark:text-white/90">
                                    {{ $paymentConcept->description }}</td>
                                <td
                                    class="px-5 py-4 sm:px-6 text-center font-medium text-gray-900 text-theme-sm dark:text-white/90">
                                    {{ $paymentConcept->type == 'I' ? 'Ingreso' : 'Egreso' }}</td>
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        @foreach ($rowOperations as $operation)
                                            @php
                                                ...
                                                $actionUrl = $resolveActionUrl($action, $paymentConcept, $operation);
        ...
        <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex-none pagination-simple">
                {{ $paymentConcepts->links('vendor.pagination.forced') }}
            </div>
        </div>
    ...
    <x-ui.modal x-data="{ open: false }" @open-create-payment-concept-modal.window="open = true"
        ...
        <div class="p-6 space-y-4">
            ...
            @endif
            <form id="create-payment-concept-form" class="space-y-4"
                action="{{ $viewId ? route('admin.payment_concepts.store') . '?view_id=' . $viewId : route('admin.payment_concepts.store') }}"
                method="POST">
                @csrf
                @if ($viewId)
                    <input type="hidden" name="view_id" value="{{ $viewId }}">
